...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
        ...
        }else if (i >= io_hole_start && i < kernel_end_page) {
            pages[i].pp_ref = 1;
            pages[i].pp_link = NULL;
        }else {
            pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
    ...
    //
    struct PageInfo* rest = page_free_list;
    if (rest == NULL) {
        cprintf("page_alloc : out of memory");
        return NULL;
    }
    page_free_list = rest -> pp_link;
    rest -> pp_link = NULL;

    if (alloc_flags && ALLOC_ZERO) {
        memset(page2kva(rest), 0, PGSIZE);
    }
    return rest;
}
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref != 0 || pp->pp_link != NULL)
        panic("Page_free pp -> ref != 0 || pp -> pp_link != NULL");
...
{
	// Fill this function in
    // PDX get the page directory using virtual address
    // PTX page table index
    // PDX page directory index
    // pde_t page direcotry point
    // pte_t page table point 
    pde_t *pt = pgdir + PDX(va);
    pde_t *pt_addr_v;
    if (*pt & PTE_P) {
        pt_addr_v = (pde_t *)KADDR(PTE_ADDR(*pt));
